...
export const AboutHBOT = () => {
  return (
    <section id="hbot" className="relative w-full py-24 overflow-hidden flex items-center justify-center">
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          <span className="text-sm font-bold uppercase tracking-wider mb-2 block text-white">Tudomány a háttérben</span>
          <h2 className="text-3xl md:text-5xl font-bold mb-6 text-white">
            Mire jó az <span className="">oxigénterápia?</span>
          </h2>
        ...
        >
          <motion.div variants={itemVariants} className="lg:col-span-12 bg-[#D2E9E9] rounded-3xl p-8 border border-brand-light/20 shadow-sm hover:shadow-lg transition-shadow duration-300">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              <div className="w-16 h-16 rounded-2xl bg-brand-primary/10 flex items-center justify-center shrink-0">
                <Gauge className="w-8 h-8 " />
              </div>
              <div>
                <h3 className="text-2xl font-bold mb-3">A működési elv: Henry törvénye</h3>
                <p className="leading-relaxed">
                  Normál légköri nyomáson az oxigént a vörösvértestek szállítják. A HBOT kabinban uralkodó túlnyomás alatt azonban – a fizikai Henry-törvény értelmében – az oxigén <span className="font-semibold">közvetlenül a vérplazmában oldódik fel</span>. Így a keringési zavarral küzdő területekre is eljut az energia.
                </p>
              </div>
            </div>
          </motion.div>
          ...
          {benefits.map((item, index) => (
            <motion.div
              ...
              className="lg:col-span-6 xl:col-span-3 bg-[#D2E9E9] rounded-3xl p-6 border border-brand-light/20 shadow-sm hover:shadow-xl transition-all duration-300 h-full flex flex-col"
            >
              <div className="w-12 h-12 bg-brand-bg rounded-xl flex items-center justify-center mb-5 ">
                <item.icon className="w-6 h-6" />
              </div>
              <h3 className="text-xl font-bold mb-3">{item.title}</h3>
              <p className="text-sm mb-5 min-h-[40px]">{item.desc}</p>
